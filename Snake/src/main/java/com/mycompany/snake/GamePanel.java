...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public void checkCollisions() {
        ...
        for (int i = bodyParts; i > 0; i--) { // ✅ fixed loop
            if ((x[0] == x[i]) && (y[0] == y[i])) {
                running = false;
            }
        }
        ...
        if (x[0] < 0 || x[0] >= SCREEN_WIDTH) { // ✅ fixed >=
            running = false;
        }
        // check if head touches top/bottom border
        if (y[0] < 0 || y[0] >= SCREEN_HEIGHT) { // ✅ fixed >=
            running = false;
        }
